...
const StudentLogin: React.FC<StudentLoginProps> = ({
  ...
  const handleLoginClick = async () => {
    ...
    try {
      ...
      if (data?.user?.role !== 'student') {
        setError('Only student are allowed to log in.');
        return;
      }
      ...
      const termsAccepted = localStorage.getItem('studentTermsAccepted');
  ...
  const handleAcceptTerms = () => {
    localStorage.setItem('studentTermsAccepted', 'true');
  ...
  return (
    <>
      <SciQuestLogo />
      <p className="mt-2 text-gray-300 text-center font-semibold text-lg">{t('studentPortal')}</p>
